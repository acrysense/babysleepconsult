...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // graduates regions accordeon
  var graduatesRegionsAccordeonTrigger = document.querySelectorAll('.graduates-regions-accordeon__trigger');
  if (graduatesRegionsAccordeonTrigger) {
    graduatesRegionsAccordeonTrigger.forEach(function (item) {
      item.addEventListener('click', function (event) {
        event.preventDefault();
        if (!item.parentNode.classList.contains('is--open')) {
          slideDownQna(item.nextElementSibling);
  ...
  if (professionTabsItems && professionTabsItems.length > 0) {
    ...
    var startAutoSwitch = function startAutoSwitch() {
      var currentIndex = 0;
  ...
  var webinarWrapper = document.querySelector('.webinar__wrapper');
  var webinarPopup = document.querySelector('.webinar__popup');
  function webinarSetHeight() {
    webinarWrapper.style.minHeight = webinarPopup.getBoundingClientRect().height + 'px';
  }
  if (webinarWrapper && webinarPopup && webinarTrigger) {
    webinarSetHeight();
    webinarTrigger.addEventListener('click', function (event) {
      event.preventDefault();
      if (!webinarWrapper.classList.contains('is--active')) {
        webinarWrapper.classList.add('is--active');
      }
    });
  }
  ...
  if (certificatesSlider) {
    var myCertificatesSlider = new Swiper(certificatesSlider, {
      ...
      loop: true,
      navigation: {
        prevEl: certificatesSlider.closest('.certificates__slider').querySelector('.swiper-button-prev'),
        nextEl: certificatesSlider.closest('.certificates__slider').querySelector('.swiper-button-next')
      },
      breakpoints: {
        0: {
          slidesPerView: 'auto',
  ...
  // gallery slider
  var gallerySlider = document.querySelectorAll('.gallery__slider .swiper');
  if (gallerySlider) {
    gallerySlider.forEach(function (slider) {
      var myGallerySlider = new Swiper(slider, {
        slidesPerView: 1,
        loop: true,
        navigation: {
          prevEl: slider.closest('.gallery__slider').querySelector('.swiper-button-prev'),
          nextEl: slider.closest('.gallery__slider').querySelector('.swiper-button-next')
        }
      });
    });
  }
  ...
  var observer;
  if (targetSection && numberWrapper) {
